defmodule MicroPhoenix.VerifiedRoutes do
  @moduledoc false
  ...
  defp segment_ast({:"::", _, [{{:., _, [Kernel, :to_string]}, _, [dynamic]}, _]}) do
    quote do: MicroPhoenix.VerifiedRoutes.__encode_segment__(unquote(dynamic))
  end

  defp segment_ast(other) do
    raise ArgumentError,
          "invalid ~p path segment: #{Macro.to_string(other)}"
  end
  ...
  defp query_segment_ast(other) do
    raise ArgumentError,
          "invalid ~p query segment: #{Macro.to_string(other)}"
  end
